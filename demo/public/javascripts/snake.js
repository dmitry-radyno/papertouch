...
SNAKE.Snake = SNAKE.Snake || (function() {
    ...
    return function(config) {
        ...
        me.socketNavigation = function(){
            ...
            eventListener.on('down', function (x, y) {
...
SNAKE.Board = SNAKE.Board || (function() {
    ...
    return function(inputConfig) {
        ...
        me.setupPlayingField = function () {
            ...
            mySnake.socketNavigation();

            eventListener.on("init", initSocketSnake);
                /*socket.on('moveUp', initSocketSnake);
                socket.on('moveDown', initSocketSnake);
                socket.on('moveLeft', initSocketSnake);
                socket.on('moveRight', initSocketSnake);*/



            function initSocketSnake(){
                if(!socketSnakeInited) {
                    console.log('snakeStarted');
                    mySnake.rebirth();
                    me.setBoardState(2); // start the game!
                    mySnake.go();
                    socketSnakeInited = true;
                }
            }
